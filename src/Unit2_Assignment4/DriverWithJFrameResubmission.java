...
public class DriverWithJFrameResubmission implements ActionListener {
	...
	Map<Playlist, String> playlistSortOrderMap = new HashMap<>();
	...
	JLabel ratingLabel = new JLabel ("Rating"),timeLabel = new JLabel("Time"), songNumberLabel = new JLabel("Song #");
	...
	public void songsMenuSetup() {
		...
		JMenuItem option9 = new JMenuItem("List common songs between two playlists");
		...
		option6.addActionListener(e -> { // remove by title
			if (currentPlaylist != null) {
				...
				String currentOrder = playlistSortOrderMap.get(currentPlaylist);
				if (currentOrder.equals("BY_ARTIST")) {
					Collections.sort(currentPlaylist.getSongs(), new SortByArtist());
				} else if (currentOrder.equals("BY_TIME")) Collections.sort(currentPlaylist.getSongs(), new SortByTime());
		...
		option8.addActionListener(e -> { // Remove a range of songs
			if (currentPlaylist != null) {
				if (currentPlaylist.getSongs().isEmpty()) {
					JOptionPane.showMessageDialog(frame, "The playlist is empty. No songs to remove.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				try {
					// ask for the starting song number
					String startInput = JOptionPane.showInputDialog(frame, "Enter the starting song number:", "Remove Songs Range", JOptionPane.QUESTION_MESSAGE);
					if (startInput == null || startInput.trim().isEmpty()) {
						JOptionPane.showMessageDialog(frame, "Invalid input. Please enter a valid starting number.", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}

					int start = Integer.parseInt(startInput.trim());

					// ask for the ending song number
					String endInput = JOptionPane.showInputDialog(frame, "Enter the ending song number:", "Remove Songs Range", JOptionPane.QUESTION_MESSAGE);
					if (endInput == null || endInput.trim().isEmpty()) {
						JOptionPane.showMessageDialog(frame, "Invalid input. Please enter a valid ending number.", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}

					int end = Integer.parseInt(endInput.trim());

					// check if invalid 
					int totalSongs = currentPlaylist.getSongs().size();
					if (start < 1 || end < 1 || start > totalSongs || end > totalSongs || start > end) {
						JOptionPane.showMessageDialog(frame, "Invalid range. Ensure 1 <= start <= end <= " + totalSongs + ".", "Error", JOptionPane.ERROR_MESSAGE);
						return;
					}

					List<Song> songs = currentPlaylist.getSongs();
					List<Song> removedSongs = new ArrayList<>(songs.subList(start - 1, end));
					songs.subList(start - 1, end).clear();
					...
					// repaint visuals
					numOfSongs.setText(String.valueOf(currentPlaylist.getNumOfSongs()));
					...
					// Success message
					JOptionPane.showMessageDialog(frame, "Removed songs from #" + start + " to #" + end + ". Total removed: " + removedSongs.size(), "Success", JOptionPane.INFORMATION_MESSAGE);
				} catch (NumberFormatException ex) {
		...
		option9.addActionListener(e -> { // Find common songs between two playlists
			if (allPlaylists.size() < 2) {
				JOptionPane.showMessageDialog(frame, "At least two playlists are required to compare.", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			}

			try {
				// ask the user for the first playlist number
				String playlist1Input = JOptionPane.showInputDialog(frame, "Enter the number of the first playlist (1-" + allPlaylists.size() + "):", "Common Songs", JOptionPane.QUESTION_MESSAGE);
				if (playlist1Input == null || playlist1Input.trim().isEmpty()) {
					JOptionPane.showMessageDialog(frame, "Invalid input. Please enter a valid playlist number.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				int playlist1Index = Integer.parseInt(playlist1Input.trim()) - 1;
				if (playlist1Index < 0 || playlist1Index >= allPlaylists.size()) {
					JOptionPane.showMessageDialog(frame, "Invalid playlist number. Please enter a number between 1 and " + allPlaylists.size() + ".", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				// ask the user for the second playlist number
				String playlist2Input = JOptionPane.showInputDialog(frame, "Enter the number of the second playlist (1-" + allPlaylists.size() + "):", "Common Songs", JOptionPane.QUESTION_MESSAGE);
				if (playlist2Input == null || playlist2Input.trim().isEmpty()) {
					JOptionPane.showMessageDialog(frame, "Invalid input. Please enter a valid playlist number.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				int playlist2Index = Integer.parseInt(playlist2Input.trim()) - 1;
				if (playlist2Index < 0 || playlist2Index >= allPlaylists.size()) {
					JOptionPane.showMessageDialog(frame, "Invalid playlist number. Please enter a number between 1 and " + allPlaylists.size() + ".", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				// make the user selected two different playlists
				if (playlist1Index == playlist2Index) {
					JOptionPane.showMessageDialog(frame, "You must select two different playlists.", "Error", JOptionPane.ERROR_MESSAGE);
					return;
				}

				Playlist playlist1 = allPlaylists.get(playlist1Index);
				Playlist playlist2 = allPlaylists.get(playlist2Index);

				Collections.sort(playlist1.getSongs(), new SortByAll());
				Collections.sort(playlist2.getSongs(), new SortByAll());

				// Find common songs using binary search
				List<Song> commonSongs = new ArrayList<>();
				for (Song song : playlist1.getSongs()) {
					int index = Collections.binarySearch(playlist2.getSongs(), song, new SortByAll());
					if (index >= 0) {
						commonSongs.add(song);
					}
				}

				// Display the common songs (JOptionPane)
				if (commonSongs.isEmpty()) {
					JOptionPane.showMessageDialog(frame, "No common songs found between the selected playlists.", "No Common Songs", JOptionPane.INFORMATION_MESSAGE);
				} else {
					StringBuilder commonSongsList = new StringBuilder("Common Songs:\n");
					for (Song song : commonSongs) {
						commonSongsList.append(song.getTitle())
						.append(" by ").append(song.getArtist())
						.append(" (Genre: ").append(song.getGenre())
						.append(", Rating: ").append(song.getRating())
						.append(", Time: ").append(song.getTime()).append(")\n");
					}
					JOptionPane.showMessageDialog(frame, commonSongsList.toString(), "Common Songs", JOptionPane.INFORMATION_MESSAGE);
				}

			} catch (NumberFormatException ex) {
				JOptionPane.showMessageDialog(frame, "Invalid input. Please enter valid playlist numbers.", "Error", JOptionPane.ERROR_MESSAGE);
			}
		});
		...
		option1.addActionListener(e -> { // Sort the songs by title
			if (currentPlaylist != null) { // check if playlist is selected
				...
				playlistSortOrderMap.put(currentPlaylist, "BY_TITLE");
		...
		option2.addActionListener(e -> { // Sort by artist
			if (currentPlaylist != null) {
				Collections.sort(currentPlaylist.getSongs(), new SortByArtist());
				playlistSortOrderMap.put(currentPlaylist, "BY_ARTIST");
				// After sorting, update and display the songs
				displaySongs(currentPlaylist);
			} else {
				JOptionPane.showMessageDialog(frame, "No playlist selected.", "Error", JOptionPane.ERROR_MESSAGE);
		...
		option3.addActionListener(e -> { // Sort by time
			if (currentPlaylist != null) {
				...
				playlistSortOrderMap.put(currentPlaylist, "BY_TIME");
				displaySongs(currentPlaylist);
			} else {
				JOptionPane.showMessageDialog(frame, "No playlist selected.", "Error", JOptionPane.ERROR_MESSAGE);
			}
		...
		// TODO
		JPanel songNumberPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
		songNumberPanel.add(songNumberLabel);
		songNumberPanel.setPreferredSize(new Dimension(100,50));
		songNumberLabel.setHorizontalAlignment(SwingConstants.LEFT);
		// END TODO
		...
		songsMenuPanel.add(songNumberPanel);
	...
	public DriverWithJFrameResubmission() throws IOException {
	...
	public static void main(String[] args) throws IOException {
		new DriverWithJFrameResubmission();
	}
	...
	public void displaySongs(Playlist playlist) {
		...
		for (int i = 0; i < playlist.getSongs().size(); ++i) {
			...
			numberPanel.setPreferredSize(new Dimension(100,50));
			...
			option1.addActionListener(e -> {
				...
				displaySongs(playlist); // Re-display all songs in the playlist

				// Repaint and revalidate the updated panel
				allSongsPanel.repaint();
				allSongsPanel.revalidate();
			});
		}
