...
public class Ex08a {
	...
	public static String encryptMessage(String original, int position) {
		String newString = "";
		for (int i = 0; i < original.length(); ++i) {
			if (65 <= original.charAt(i) && original.charAt(i) <= 90) {
				if (original.charAt(i)+position > 90) newString += (char) (original.charAt(i)+position-26);
				else newString += (char) (original.charAt(i)+position);

			} else if (97 <= original.charAt(i) && original.charAt(i) <= 122){
				if (original.charAt(i)+position > 122) newString += (char) (original.charAt(i)+position-26);
				else newString += (char) (original.charAt(i)+position);
 ...
 */
		...
		System.out.println("Your encrypted message is: " + encryptMessage(s,pos));
		in.close();
	}
}
